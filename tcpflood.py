...
def main():
    import sys
    if len(sys.argv) < 5:
        print(cyan_to_blue("> Usage: python tcpflood.py <ip> <threads> <duration> <port>"))
        sys.exit(1)
    ...
    print(cyan_to_blue(f"> Launching TCPFLOOD attack on {target_ip}:{target_port} with {threads} threads for {duration} seconds..."))
    ...
    time.sleep(duration)
    event.clear()
    print(cyan_to_blue("> Attack TCPFLOOD finished."))
